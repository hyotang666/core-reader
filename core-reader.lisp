...
(define-compiler-macro char-pred (&whole whole char &optional check)
  (if (constantp char)
      (if (constantp check)
          (let ((char (eval char)) (check (eval check)) (var (gensym "CHAR")))
            (check-type char character)
            (if check
                `(flet ((char-pred (,var)
                          (and (characterp ,var) (char= ,char ,var))))
                   (declare
                     (ftype (function (t) (values boolean &optional))
                            char-pred))
                   #'char-pred)
                `(flet ((char-pred (,var)
                          (char= ,char ,var)))
                   (declare
                     (ftype (function (character) (values boolean &optional))
                            char-pred))
                   #'char-pred)))
          (let ((char (eval char)) (var (gensym "CHAR")))
            (check-type char character)
            `(if ,check
                 (flet ((char-pred (,var)
                          (and (characterp ,var) (char= ,char ,var))))
                 ...
                 (flet ((char-pred (,var)
                   ...
                   #'char-pred))))
      (if (constantp check)
          (let ((check (eval check)) (c (gensym "CHAR")) (var (gensym "CHAR")))
            (if check
                `(let ((,c ,char))
                   (check-type ,c character)
                   (flet ((char-pred (,var)
                            (and (characterp ,var) (char= ,c ,var))))
                     (declare
                       (ftype (function (t) (values boolean &optional))
                              char-pred))
                     #'char-pred))
